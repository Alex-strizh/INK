      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-gray-100 flex flex-col">
        ...
        <div className="p-6 space-y-6 flex-1">
          ...
          {tool.operations && tool.operations.length > 0 && (
            <div>
              <h3 className="text-xs font-bold uppercase text-gray-400 tracking-wider mb-2">Технологические операции</h3>
              <div className="flex flex-wrap gap-1.5">
                {tool.operations.map(op => (
                  <span key={op} className="text-xs font-bold px-2.5 py-1 bg-blue-50 text-blue-700 rounded-lg border border-blue-100 shadow-sm">✓ {op}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="p-4 border-t bg-gray-50 rounded-b-2xl flex flex-wrap justify-between items-center gap-3">
          <a href={`/pdf/${pdfFileName}`} download target="_blank" rel="noopener noreferrer" className="px-4 py-2 border rounded-xl bg-white text-sm font-semibold text-blue-600 border-blue-200 hover:bg-blue-50 transition-colors shadow-sm flex items-center gap-2">
            📄 PDF Каталог серии
          </a>
